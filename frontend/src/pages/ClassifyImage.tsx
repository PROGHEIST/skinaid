...
const ClassifyImage = () => {
  ...
  const handleClassify = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Server returned status ${response.status}. Make sure the ML backend is running on port 8001.`);
      }
      ...
      toast({
        ...
        description: `Top Prediction: ${sortedPredictions[0].label}${noteMsg}`,
      });
